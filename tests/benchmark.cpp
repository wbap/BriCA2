...
static void ConstPipeNullParallel(benchmark::State& state) {
  std::size_t size = static_cast<std::size_t>(state.range_x());
  ...
  brica2::core::Agent agent;
  brica2::schedulers::VirtualTimeSyncScheduler s(agent, 1.0, 4);
...
static void ConstPythonNullParallel(benchmark::State& state) {
  try {
    std::size_t size = static_cast<std::size_t>(state.range_x());
...
BENCHMARK(ConstPipeNullParallel)->Range(4, 1 << 20);
BENCHMARK(ConstPythonNull)->Range(4, 1 << 20);
BENCHMARK(ConstPythonNullParallel)->Range(4, 1 << 20);
